...
export default function Friend(props) {
  ...
  const { friendObj, changeStatusFunc } = props;
  return (
    <div className='friend-friends container'>
      ...
      <div className='friend-info'>
        <div >
          ...
          <p>Married: {friendObj.married ? 'Yes' : 'No'} <button onClick={()=> changeStatusFunc(friendObj.id)}>change</button></p>
          <div>Likes:
            <ul>
              {/* 👉 3- Loop over the friend's hobbies and generate <li/> elements as you go */}
              {friendObj.hobbies.map((hobby, i) => {
